...
Settlement returns a deduplicated top-level `titleUpdates` delta built from processed events, approval finalization, and shared or isolated activation. The caller must apply every entry through the Codex app title tool before replying and must not resubmit unchanged titles from the final queue. The delta includes every task whose projected state title or visible queued position may have changed, so user-attention changes update `👉`, while enqueue, move, activation, block, pause, return-to-planning, resume, cancellation, exclusion, or completion still renumbers all affected `QUEUED` tasks from `①` without counting `RUNNING`, `REVIEW`, `APPROVED`, or `BLOCKED` tasks. `PAUSED` remains outside the active queue. Returned-to-planning, paused, completed, canceled, and registered-excluded tasks remain included even when absent from the final queue: `PLANNING` receives `⚪️`, `PAUSED` receives `⏸️`, `DONE` keeps its returned `🟢` title, and `CANCELED` is reset to the semantic name with no icon, queue marker, or task ID. Retry one failed title operation once, then surface the exact failure. An explicit queue read provides the full title projection for recovery.
