...
- `Approve`: accept only in `REVIEW` and only from the user's direct message in that worker. Treat the direct command or a clear language equivalent as final authorization, never as a request that needs another confirmation. Immediately generate a concise, meaningful English imperative commit subject describing the final implementation, never the task ID, title, or semantic name, submit `APPROVAL_REQUESTED` with `--commit-message`, and settle. Do not repeat the independent-review offer or ask the user to confirm the commit or integration. If the task is not in `REVIEW`, reject once with its exact current state and the required next action.
