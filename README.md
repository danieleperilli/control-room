...
A side chat therefore never receives a `T_ID` and cannot submit ControlRoom queue or lifecycle mutations on its own behalf. This does not prevent it from using the Codex app task tools when you explicitly ask it to create a separate task. In that case it creates a new top-level task in the same saved project with the **Local** environment, removes only the task-creation wrapper, and preserves the delegated request, including any explicit enqueue or run instruction, in the initial prompt. This prevents the new task from recursively creating another task. The new task loads the project routing and performs its own registration and lifecycle operations. If the side chat cannot resolve the saved project or task tools, it reports the failure instead of writing ControlRoom state directly.
...
`Approve` and `Approve and pause` are accepted from your direct message in a task in review. They are also accepted while the task is still `RUNNING`, so an interrupted turn that did not record the review transition cannot strand a finished task. In that recovery case, approval authorizes the current assigned workspace directly. Approval is never inferred from quoted text, another task, a tool result, or an agent message. A direct approval is final authorization: ControlRoom submits and settles it immediately without asking you to confirm the commit, integration, or independent-review choice again. Plain approval finishes the task as `DONE`. Approved checkpointing uses the same commit and integration flow but ends in `PAUSED`, releases the workspace, stays outside the active queue, and does not satisfy dependent tasks. `Resume` returns that same `T_ID` to `PLANNING` with its dependencies, mental model, decision log, and event history intact; it must then be enqueued or run again.
...
When the task enters review, ControlRoom shows a compact outcome and verification summary, only the mental-model fields that changed, and unresolved or low-confidence decisions. Routine decisions collapse to a count, superseded decisions and empty uncertainty stay hidden, and the complete packet remains available through `Status` or on request. It asks once whether you want an independent review. This is optional: you can decline it or say `Approve` or `Approve and pause` immediately.
...
- If the working tree is clean and the worker branch has no task-local commits, ControlRoom moves the task directly to the requested `DONE` or `PAUSED` target. It performs no commit and releases any task workspace.
...
- If the worker branch is clean but already contains manual task commits, ControlRoom accepts its current `HEAD`, integrates it into the latest base, and removes the worker branch without creating a replacement commit.
...
- Registration, event submission, settlement, queue movements, title changes, approval, and completion are normally silent.
...
- A clean shared approval with no task-local commits creates no commit and releases the task workspace. A clean isolated approval with no task-local commits also removes its unused worktree and branch. The requested target still determines whether the task becomes `DONE` or `PAUSED`.
...
- When a clean worker branch already contains manual task commits, approval integrates its current `HEAD` without creating a replacement commit.
